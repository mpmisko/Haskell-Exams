...
checkSat (root, nodes) e = checkSat' root
  where
    checkSat' curNode
      | curNode == 1 = True
      | curNode == 0 = False
      | otherwise    = checkSat' nextNode
      where
        (index, l, r) = lookUp curNode nodes
        direction     = lookUp index e
        nextNode      = if direction then r else l
  ...
  = sat' root
    where
      sat' curIndex
        ...
        where 
          (index, l, r) = lookUp curIndex nodes
          left = appendAll (index, False) (sat' l)
          right = appendAll (index, True) (sat' r)
          appendAll x xs = [(x : y) | y <- xs]
------------------------------------------------------
-- PART II

simplify :: BExp -> BExp
simplify (Prim b) = Prim b 
simplify (IdRef i) = IdRef i
...
restrict (IdRef index) i b
  | i == index = Prim b
  | otherwise  = IdRef index
...
buildBDD exp indexes
  = buildBDD' exp 2 indexes
...
buildBDD' (Prim b) _ []
  | b         = (1, [])
  | otherwise = (0, [])
buildBDD' exp id (i : is) = (id, ((id, (i, l, r)) : (rest ++ rest'))) 
  where
    (l, rest) = (buildBDD' (restrict exp i False) (2*id) is)
    (r, rest') = (buildBDD' (restrict exp i True) (2*id + 1) is)
